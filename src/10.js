...
window.GameTest={
 ...
 validate:()=>{const state=window.__battleSim.state(),companyMax=Math.max(0,...state.companies.flat().map(c=>c.men)),finite=[...state.money,...state.fortresses.flatMap(f=>[f.hp,f.maxHp]),state.frontline].every(Number.isFinite),negativeTreasury=state.money.some(v=>v<-.001),overCapacity=companyMax>MAX_PER_COMMANDER,invalidFort=state.fortresses.some(f=>f.hp<-.001||f.hp>f.maxHp+.001),invalidRank=actors.some(a=>a.alive&&!a.isCommander&&!PURCHASABLE_RANKS.includes(classOf(a)));return{ok:finite&&!negativeTreasury&&!overCapacity&&!invalidFort&&!invalidRank,finite,negativeTreasury,overCapacity,invalidFort,invalidRank,companyMax,maxPerCommander:MAX_PER_COMMANDER,roadmapCount:roadmap.length,version:state.version}},
