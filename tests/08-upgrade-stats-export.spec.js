...
test('General AI training is blocked during the first 900 seconds so early rank ecology keeps priority',async({page})=>{
  await openGame(page);
  const r=await page.evaluate(()=>{
    GameTest.setSeed(36002);const api=__battleSim.test,g=api.generals()[0];g.money=10000;g.stance='ATTACK';g.reserveTarget=80;for(let i=0;i<18;i++)api.buyMusketeer(0,'F');g.money=10000;g.nextStatUpgradeAt=0;
    const before={...g.stats},early=api.tryArmyTraining(0),state=GameTest.state();return{before,early,after:{...g.stats},maturity:state.statTraining.aiMaturitySeconds,upgrades:g.statUpgrades,spend:g.statSpend,validation:GameTest.validate()};
  });
...
test('Export State downloads a complete current-state JSON immediately without advancing the battle',async({page},testInfo)=>{
  ...
  expect(download.suggestedFilename()).toMatch(/^musketeer-state-v3\.4\.1-seed-36003-war-1-t-0s\.json$/);expect(payload.game).toBe('Musketeer Battle Simulator');expect(payload.version).toBe('3.4.1');expect(payload.runtime.seed).toBe(36003);expect(payload.runtime.simTime).toBe(0);expect(Number.isInteger(payload.runtime.rngState)).toBe(true);
  expect(payload.state.patchVersion).toBe('3.4.1');expect(payload.state.armyStats[0].Offense).toBe(1);expect(payload.state.companyReform.startsAt).toBe(900);expect(Array.isArray(payload.actors)).toBe(true);expect(payload.actors.length).toBeGreaterThan(0);expect(Array.isArray(payload.generals)).toBe(true);expect(Array.isArray(payload.positions)).toBe(true);expect(Array.isArray(payload.flightRecorder)).toBe(true);expect(payload.anomalies.longWarNoWinner).toBe(false);
  await testInfo.attach('instant-export-v341-example.json',{body:Buffer.from(JSON.stringify(payload,null,2)),contentType:'application/json'});
});