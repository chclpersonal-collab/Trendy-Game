...
test('wiped rich army reports MUSTER BLOCKED during enemy BREACH and recovers after relief',async({page})=>{
  ...
  const r=await page.evaluate(()=>{
    GameTest.setSeed(33005);const api=__battleSim.test,attacker=api.generals()[0],defender=api.generals()[1];attacker.money=1000;defender.money=2305;
    for(let i=0;i<6;i++)api.buyMusketeer(0,'F');
    GameTest.forceBreach(0,0,true);api.generalDecision(1);
    const blocked={plan:defender.budgetPlan,army:GameTest.state().musketeers[1],money:defender.money,fieldwork:fieldworkMusterBlocked(1)};
