...
test('v3.5 exposes ranked economy, HP/mana/range profiles, real army training, siege diagnostics, and C/Makashi metadata',async({page})=>{
  ...
  expect(r.version).toBe('3.5');expect(r.phase).toBe(4);expect(r.patchVersion).toBe('3.5.0');
  expect(r.rework).toBe('ranked-economy-hp-mana-range-screening');
  ...
  expect(r.economy.maintenanceRemoved).toBe(true);expect(r.rankSystem.order).toHaveLength(15);expect(r.rankSystem.profiles.F.mana).toBe(0);expect(r.rankSystem.profiles.C.range).toBeGreaterThan(r.rankSystem.profiles.D.range);
  ...
  await expect(page.locator('#exportStateBtn')).toBeVisible();expect(await page.title()).toContain('Phase 4 v3.5');expect(await page.locator('.version').textContent()).toContain('v3.5');
});
