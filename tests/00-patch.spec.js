...
test('v3.4 exposes real army training, siege resolution, export diagnostics, and preserves C/Makashi metadata',async({page})=>{
  ...
  expect(r.version).toBe('3.4');expect(r.phase).toBe(4);expect(r.patchVersion).toBe('3.4.0');
  expect(r.rework).toBe('upgradeable-army-stats-siege-resolution-state-export');
  ...
  await expect(page.locator('#exportStateBtn')).toBeVisible();expect(await page.title()).toContain('Phase 4 v3.4');expect(await page.locator('.version').textContent()).toContain('v3.4');
});