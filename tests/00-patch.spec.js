...
test('v3.2 Makashi and tactical-combat-continuity metadata is live',async({page})=>{
  ...
  expect(r.patchVersion).toBe('3.2.0');
  expect(r.rework).toBe('makashi-tactical-combat-continuity');
  ...
  expect(await page.title()).toContain('Phase 3 v3.2');
  expect(await page.locator('.version').textContent()).toContain('v3.2');
});
