...
async function openGame(page) {
  ...
  if ((await pauseButton.textContent())?.trim() === 'Pause') {
...
test('300-second deterministic browser self-play remains valid', async ({ page }, testInfo) => {
  ...
  await testInfo.attach('state-300s.json', {
    body: Buffer.from(JSON.stringify(result, null, 2)),
    contentType: 'application/json'
  });
});

test('forced BREACH can convert into real fortress damage', async ({ page }, testInfo) => {
  await openGame(page);
  const result = await page.evaluate(() => {
    window.GameTest.setSeed(21819);
    window.GameTest.forceBreach(0, 0);
    const before = window.GameTest.snapshot();
    const after = window.GameTest.advance(60);
  ...
  await testInfo.attach('forced-breach.json', {
    body: Buffer.from(JSON.stringify(result, null, 2)),
    contentType: 'application/json'
  });
});
